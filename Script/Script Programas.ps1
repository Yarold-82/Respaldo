...
Write-Host "        Created with UniGetUI Version 2026.1.0"
Write-Host ""
Write-Host "========================================================"
Write-Host ""
...
Write-Host "  - Filezilla from Chocolatey"
Write-Host "  - VeraCrypt from WinGet"
Write-Host "  - MobaXterm from WinGet"
Write-Host "  - Microsoft Visual Studio Code from WinGet"
Write-Host "  - Remote Desktop Manager from WinGet"
...
$commands= @(
    ...
    'cmd.exe /C choco.exe install filezilla -y --no-progress',
    ...
    'cmd.exe /C winget.exe install --id "Devolutions.RemoteDesktopManager" --exact --source winget --accept-source-agreements --disable-interactivity --silent --accept-package-agreements --force',
